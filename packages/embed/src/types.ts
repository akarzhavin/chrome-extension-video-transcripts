...
export interface EmbedOptions {
    ...
    /** Poster shown before playback starts (file source only). */
    poster?: string;
     ...
     * setMode(). Lets the host page mirror the demo's state (e.g. a mode
     * explainer that follows what the visitor clicks in the panel).
     */
    onModeChange?: (mode: 'dual' | 'single' | 'guess') => void;

    /**
     * Fires when another copy of the Lingogram UI already owns the #vtt-* ids
     * on this page — in practice, the installed browser extension, which
     * injects everywhere including this host. The embed renders no UI in that
     * case (sharing the ids would produce controls that click but drive
     * nothing), so the host should explain the collision to the visitor.
     */
