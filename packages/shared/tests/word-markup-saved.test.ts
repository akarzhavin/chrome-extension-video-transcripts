...
describe('guess mode', () => {
    test('a revealed saved word carries the mark', () => {
        ...
        fillMaskedWordsInto(el, LINE, 99, undefined, savedIsBeautiful);
    ...
    test('a masked saved word is byte-identical to a masked unsaved one', () => {
        ...
        fillMaskedWordsInto(withSave, LINE, 0, undefined, savedIsBeautiful);

        const withoutSave = container();
        fillMaskedWordsInto(withoutSave, LINE, 0, undefined, savedIsNothing);
    ...
    test('the mark appears exactly when the word is uncovered', () => {
        ...
        fillMaskedWordsInto(hidden, LINE, 3, undefined, savedIsBeautiful);
        expect(marks(hidden)).toEqual([]);

        const shown = container();
        fillMaskedWordsInto(shown, LINE, 4, undefined, savedIsBeautiful);
        expect(marks(shown)).toEqual(['beautiful']);
    });

    test('a word opened out of order is marked, and its neighbours are not', () => {
        // The seam between the two features that met in this merge: words can
        // be uncovered by pointing at them, so "which words are out" is the
        // isRevealed predicate and NOT `m < revealedCount`. A mark keyed off
        // the count would appear on a capsule as soon as any earlier word was
        // picked — here the count is 0 and only token 3 is open.
        const el = container();
        const onlyBeautiful = (ti: number): boolean => ti === 3;

        fillMaskedWordsInto(el, LINE, 0, onlyBeautiful, savedIsBeautiful);

        expect(marks(el)).toEqual(['beautiful']);
        // And the still-hidden words carry nothing at all.
        expect(el.querySelectorAll(`.vtt-masked-word.${SAVED_MARK_CLASS}`).length).toBe(0);
    });

    test('a saved word still hidden while others are picked stays bare', () => {
        // The mirror image: "beautiful" is saved but NOT the picked word, so
        // it is still under its capsule and must look like any other capsule.
        const el = container();
        const onlyEvening = (ti: number): boolean => ti === 4;

        fillMaskedWordsInto(el, LINE, 0, onlyEvening, savedIsBeautiful);
