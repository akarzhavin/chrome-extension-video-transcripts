...
test.describe('the settings screen', () => {
    test('the settings screen opens over the transcript and closes again', async ({ ext }) => {
     ...
     */
    test('reset returns a changed setting to its default', async ({ ext }) => {
        await preservingUiPrefs(ext, async () => {
            const page = await ext.open(VIDEO);
            try {
                await waitForLines(page);
...
test.describe('exporting subtitles', () => {
     ...
     */
    test('the export offer names the loaded language', async ({ ext }) => {
        await preservingUiPrefs(ext, async () => {
            const page = await ext.open(VIDEO);
            try {
                await waitForLines(page);
