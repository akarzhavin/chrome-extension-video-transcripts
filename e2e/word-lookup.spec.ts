...
/**
 * All of this runs on every platform: the card, the selection rule and the
 * click-to-seek are the panel's own behaviour, asserted against `#vtt-list`
 * and the card's own markup. One check parameterised over `SITES`, never a copy
 * per site.
 */
...
for (const site of SITES) {
    test.describe(`${site.name}: looking a word up`, () => {
         ...
         */
        test('with no languages set, there is no transcript to look a word up in', async ({ ext }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');

            // This one needs a page loaded AFTER the languages are cleared, so
            // it opens its own rather than taking the shared one — the setup
            // gate is decided at injection. `site.open` is what keeps that
            // platform-free.
            await withPrefs(ext, null, async () => {
                const fresh = await site.open(ext);
                try {
                    await expect
...
for (const site of SITES) {
    test.describe(`${site.name}: selecting and copying`, () => {
         ...
         */
        test('a selection inside the translation opens no card', async ({ ext, pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);
...
for (const site of SITES) {
    test.describe(`${site.name}: the transcript as a navigation control`, () => {
        /** Behaviour map §38: a press on a line moves playback to it. */
        test('clicking a line moves playback to that line', async ({ pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);

            await waitForLines(page);
            await playFrom(page, 5, site);
