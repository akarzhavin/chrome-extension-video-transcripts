...
/**
 * Four of the seven groups here are about the panel's own settings screen and
 * run on every platform — one check parameterised over `SITES`, not a copy per
 * site. Three stay YouTube-only, each for a stated reason:
 *
...
for (const site of SITES) {
    test.describe(`${site.name}: moving the captions`, () => {
        /**
         ...
         */
        test('the drag grip is only offered while the settings screen is open', async ({ ext, pageFor }) => {
...
for (const site of SITES) {
    test.describe(`${site.name}: the style rows and what they offer`, () => {
        /**
         ...
         * to, silently.
         */
        test('the typeface list offers the seven CEA-708 classes', async ({ ext, pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);
         ...
         */
        test('both text colour rows carry five swatches and a custom well', async ({ ext, pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);
         ...
         */
        test('the opacity row offers exactly four labelled steps', async ({ ext, pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);
         ...
         */
        test('the drag grip carries a name', async ({ ext, pageFor }) => {
            const reason = site.skipReason();
            test.skip(reason !== null, reason ?? '');
            const page = await pageFor(site);
