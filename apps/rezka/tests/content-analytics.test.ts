...
describe('a 429 from the CDN reaches analytics as throttling', () => {
    ...
    class Harness {
        ...
        // declareNoSubtitles is deliberately NOT mirrored here. It used to be,
        // and the copy kept asserting `tracks.length > 0` and
        // `lastFailure || 'not-selected'` long after production stopped doing
        // either — green over a shipped bug, the exact failure the note at the
        // top of this file warns about. Its three tests now drive the real
        // VttApp in native-only-banner.test.ts.
    }
    ...
    // The three no_subtitles tests that used to live here (empty-failure
    // bucket, a real failure carrying through, the language pair) moved to
    // native-only-banner.test.ts, where they run against the real
    // declareNoSubtitles instead of a copy of it.
});
