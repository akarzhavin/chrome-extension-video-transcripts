...
**T413 — «drop the vacuous half» в `word-lookup.spec.ts`.** Я сначала отметил
её сделанной, не проверив, и снял отметку. Контрольная половина в проверке есть
(выделение в изучаемом языке ДОЛЖНО открыть карточку), так что формулировка
задачи не соответствует коду. Настоящий дефект другой: проверка целиком
пропускается на фикстурном видео, потому что вторая дорожка не загружается.
Чинить надо не «половину», а условие пропуска — либо видео.
