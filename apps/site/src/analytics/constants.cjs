...
// That is why this file is plain JS and not .ts: build.mjs has no TypeScript
// pipeline (it already imports ../../packages/shared/vite-limits.mjs the same
// way), while the TS side reaches it through `allowJs`.
//
// And .cjs rather than .mjs because it has to satisfy three consumers, not
// two. Node's ESM loader name-imports from CommonJS happily, Vite bundles it,
// but Jest routes any .mjs down its native-ESM path where transforms never
// run — so an .mjs here reaches the test runner untranspiled and fails to
// parse. .cjs is the one extension all three accept.
//
...
const CONSENT_KEY = 'lingogram_consent';
...
const GA_COOKIE_EXPIRY = 'Thu, 01 Jan 1970 00:00:00 GMT';
...
const STORE_HOSTS = ['chromewebstore.google.com', 'chrome.google.com/webstore'];
...
const EDITION_FALLBACK = 'unknown';
const PLACEMENT_FALLBACK = 'other';
